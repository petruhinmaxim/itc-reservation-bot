...
export class VpnL10nRu implements VpnL10n {
    ...
    instruction(scene: s.Instruction): string {
        return (`Доступ: \n` +
            ...
            `3) Данные для подключения будут предоставлены в преветственном сообщение после успешного бронирования. Если не удается подключиться к серверу, обновите приветственное сообщение и используейте новый пароль\n \n` +
            'Инструкция: \n' +
            '1) Для подключения к серверу используйте "Подключение к удаленному рабочему столу" из стандартного пакета Windows \n' +
            '2) Не забывайте закрыть все запущенные программы после завершения тренировки \n' +
            '3) Если заканчиваете работу с сервером до окончания времени бронирования, нажмите, пожалуйста, кнопку "Отменить бронирование"\n \n' 
        )
