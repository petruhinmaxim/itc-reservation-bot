...
export class VpnL10nRu implements VpnL10n {
    ...
    iphoneInstruction(scene: s.IphoneInstruction): string {
        return 'Видеоинструкция для настройки VPN на Iphone отправлена сообщением выше или воспользуйтесь текстовой:\n \n'  +
    ...
    macInstruction(scene: s.MacInstruction): string {
        return 'Видеоинструкция для настройки VPN на MAC отправлена сообщением выше или воспользуйтесь текстовой:\n \n'  +
            "1) Скачайте и установите клиент OpenVPN. [Ссылка для скачивания](https://openvpn.net/downloads/)\n \n" +
            '2) Откройте скачаннеый ранее в Telegram файл "pccontiq.ovpn" с помощью программы OpenVPN => на вопрос «Import .ovpn profile» нажмите «ОК» => нажмите «CONNECT»  \n \n' +
            'VPN подключен. На забывайте отключать VPN, когда в нем нет потребности'
    }

    androidInstruction(scene: s.AndroidInstruction): string {
        return 'Видеоинструкция для настройки VPN на Android отправлена сообщением выше или воспользуйтесь текстовой:\n \n'  +
            "1) Скачайте и установите клиент OpenVPN. [Ссылка для скачивания](https://apps.apple.com/ru/app/openvpn-connect/id590379981)\n \n" +
